module ngSchedule {
	...
	function doneDrag(data: DragData, allCells: JQuery) {
		...
		let model = createAdScheduleViewModel();
	...
	schedulePicker.$inject = ['$log'];
	function schedulePicker(logger: ng.ILogService): ng.IDirective {
		return {
			restrict: 'E',
			require: 'ngModel',
			link: link,
...
<table>
	...
	<!--<tbody>
		...
		function link(scope: ISchedulePickerScope, element: JQuery, attrs: ISchedulePickerAttributes, ngModel: ng.INgModelController) {
			...
			ngModel.$isEmpty = scheduleIsEmpty;
			...
			ngModel.$render = function () {
				...
				if (angular.isArray(ngModel.$modelValue)) {
					...
					model = createAdScheduleViewModel();
					ngModel.$setViewValue(model);
				}

				//body.replaceChild()
				//body.empty();
				//body.append(<Element>getTableBody(hours, model));
	...
	angular.module('ngSchedule', []).directive('schedulePicker', schedulePicker);
}